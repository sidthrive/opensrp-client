...
package layout.face.sidface.activity;
...
public class ImageConfirmation extends Activity {
	...
	public SidFaceActivity faceRecog;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int angle = extras
				.getInt("org.sid.sidface.ImageConfirmation.orientation");
		boolean cameraFacingFront = extras
				.getBoolean("org.sid.sidface.ImageConfirmation.switchCamera");
		boolean throughGallery = extras
				// If the image is coming through the gallery
				.getBoolean("org.sid.sidface.ImageConfirmation.through.gallery");
		...
		// Setting the view with
		// the bitmap image that came in.
							...
							if (arrayOfRects[i].contains(xCoord, yCoord)) {
								...
								// Toast the name of the person
								{
									...
									if (faceDataArray[i].getPersonId() < 0) {
										// Check the array position corresponding the rect and add that index
										arrayPosition = i;
...
//		saveCloud(albumBuffer);
	...
	private void saveMediaFile(String var1) {
		Tools.SavePictureToFile(this, storedBitmap, var1);
	}
